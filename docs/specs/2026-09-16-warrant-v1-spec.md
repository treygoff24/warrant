...
- Ruling:
  ...
  a signed record by which a human amends approved policy, including instrument adoption and accepted limitations, or accepts specific enumerated violations, with scope, policy digest, candidate range, expiry, and supersession. Section 11.
- Exception: a ruling of kind `exception` that satisfies evaluated obligations for enumerated finding identities and their occurrences, with a disposition (`false-positive`, `accepted-design`, `temporary-debt`). Section 11.4.
- Profile: a file that carries taste: measurable preferences as visible advice or explicitly chosen acceptance requirements, plus optional typed model questions. Section 16.
...
- Warrant's own content hashing scheme. It would not be comparable to commits and would reimplement Git's established content-addressed source identity.
...
retrieval_qualification:
  tool_build_digest: "sha256:…"
  retrieval_config_digest: "sha256:…"
  qualified_on: "s7:sha256:…"
  reference_policy_digest: "sha256:…"
...
`retrieval_qualification` is result provenance, not a rule definition. Its report/sample references are bound separately in outputs and excluded from the canonical policy digest, avoiding a report that changes the policy it measured merely by being attached. The actual admitted tool build, retrieval settings, and acceptance requirements remain canonical configuration inputs. Qualification metadata cannot adopt a new build or change those requirements.
...
| Copy an old signed ruling to cover a new finding | Finding ids, occurrence-evidence digests, count bounds, `candidate_range`, and `policy_digest` are inside the signed bytes | |
...
S7 qualifies retrieval before M1 closes. `retrieval.qualified` is false until an applicable report meets the predeclared acceptance criteria; the existence of a report, including a failing one, is not qualification. Qualification binds the Warrant build, retrieval settings, S7 sample, reference policy, approved criteria, and result report through the lock's `retrieval_qualification` record. A changed build or retrieval configuration needs an applicable rerun; `self-qualify` may supply measurements under the same S7 protocol but cannot bypass explicit instrument adoption (section 11.6). Changed samples or criteria have new identities, not inherited success. This is evidence about measured scope, not a guarantee that every new query is correct. Tests cover vocabulary mismatch, multiple plausible owners, genuine novelty, unrelated prompts, stale examples and inputs, no model, and capped output. Known aliases live with domain declarations, not in a hidden synonym database.
...
| An unrelated edit changes source near a covered occurrence | The occurrence digest and exception applicability stay stable; no new signature is needed for irrelevant surrounding bytes. |
...
| A queried source file is unchanged but another model input changes | A stale graph answer is not relabeled current; all required model inputs determine freshness. |
...
The delivery cases also require S7's recorded harness runs. Synthetic adapter fixtures alone cannot establish model-input timing, installed hook trust, or actual timeout and skipped-hook behavior.
...
- git 2.40 is a retained candidate floor.

  M0 rechecks it against the capture/read operations Warrant actually uses; the retired merge-engine spike no longer justifies it. The receipt records the Git version that ran.
...
| M1. A useful map before decisions | TypeScript model, S6 parity, declaration schema and canonical policy definitions, query, manifest-free census and starter proposals, human-reviewed Atlas module/ownership declarations, context, single-change propose, Mermaid/JSON map; small Cargo package-dependency integration; one before-decision harness adapter. Resolve S1 and qualify retrieval/delivery through S7 under Trey's preapproved sample and criteria. | Ordinary-language queries return real references, reuse candidates, examples, consumers, and required evidence under S7's predeclared criteria and identified reference policy. Ambiguous, unrelated, no-match, incomplete, stale, and legitimate-new-owner cases are distinct. A recorded supported-harness run proves context preceded the decision, including nondelivery cases. Qualified resolution has zero disagreements on covered features. Cargo exercises the language boundary without full Rust internals. No capability exceeds its measured scope. |
| M2. Honest advisory checks and the early product checkpoint | Typed claims, dependency/interface/pattern and declared ownership checks, capability structure and evidence requirements; four-fact advisory verdict; visible preference advice and explicit stricter choices; signals and explanations; two ruling kinds with exact exception batches; human signing and policy-anchor chain; S4 verification. Then run section 17.5's early advisory comparison. | Stories A-C distinguish reuse suggestions, signed policy changes, and incomplete analysis. New source under unchanged rules keeps policy identity. Present-day obligation equality does not prove future equivalence; call occurrence does not prove authorization behavior. Record onboarding exception burden, re-sign requests caused by occurrence drift, and policy-change classifications. R14 then requires useful-guidance evidence: no demonstrated benefit stops progression for reconsideration, and mixed results go to Trey. This does not prove deployed runner isolation. |
