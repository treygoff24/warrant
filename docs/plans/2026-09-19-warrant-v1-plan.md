...
[reviewer]
routes = [{engine="claude", model="claude-opus-5", effort="xhigh", mode="work"}, {engine="cursor", model="cursor-grok-4.6-xhigh", effort="high", mode="work"}, {engine="omp", model="glm", effort="high", mode="work"}]
[attacker]
routes = [{engine="cursor", model="cursor-grok-4.6-xhigh", effort="high", mode="work"}, {engine="grok", model="grok-4.6", effort="high", mode="work"}]
[adjudicator]
routes = [{engine="claude", model="claude-opus-5", effort="xhigh", mode="work"}, {engine="omp", model="glm", effort="high", mode="work"}]
[verifier]
...
owned_files = ["crates/snapshot/src/native.rs", "crates/snapshot/tests/native.rs", "docs/spikes/2026-09-S3-native-capture.md"]
...
verify = [{run = "cargo test --locked -p warrant-snapshot --all-features", expect = "exit 0"}, {run = "sh -c 'rg -q \"^Status: (met|not met)\" docs/spikes/2026-09-S3-native-capture.md'", expect = "exit 0"}]
reversibility = "reversible"
effects = ["code", "docs"]
...
verify = [{run = "cargo test --locked -p warrant-cli", expect = "exit 0"}, {run = "sh -c 'cargo run -q --bin warrant -- capabilities | python3 -c \"import json,sys; d=json.load(sys.stdin); assert \\\"snapshot\\\" in d[\\\"implemented\\\"]\"'", expect = "exit 0"}, {run = "scripts/gate.sh", expect = "exit 0"}]
reversibility = "reversible"
effects = ["code"]
